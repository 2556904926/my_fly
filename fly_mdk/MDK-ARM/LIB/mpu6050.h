...
typedef struct mpu6050_init mpu6050_init;
typedef uint8_t (*mpu6050_tx)(uint8_t address,uint8_t reg,uint8_t len,uint8_t* data);
typedef uint8_t (*mpu6050_rx)(uint8_t address,uint8_t reg,uint8_t len,uint8_t* data);
typedef void (*delay)(int time);
struct mpu6050_init{
    mpu6050_tx tx;
    mpu6050_rx rx;
    delay delay_ms;
...
typedef struct 
{
  FloatXYZ deg_s;  //+_2000dps  对应short 16位
  Angle Angle;
  FloatXYZ acc_g;  //+—2g      对应short16位
  float T;//放大一百倍之后的数据
}Mpu6050_Data;
...
uint8_t mpu6050_r_len(uint8_t address,uint8_t reg,uint8_t len,uint8_t* data);
...
extern mpu6050_init mpu6050;
extern Mpu6050_Data Raw_data,Filtered_data;
#endif